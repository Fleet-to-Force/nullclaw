...
pub const LogObserver = struct {
    ...
    fn logRecordEvent(_: *anyopaque, event: *const ObserverEvent) void {
        switch (event.*) {
            ...
            .subagent_start => |e| std.log.info("subagent.start agent_name={s}", .{e.agent_name}),
            .cron_job_start => |e| {
                if (e.channel) |ch| {
...
pub const FileObserver = struct {
    ...
    fn fileRecordEvent(ptr: *anyopaque, event: *const ObserverEvent) void {
        ...
        const line = switch (event.*) {
            ...
            .subagent_start => |e| std.fmt.bufPrint(&buf, "{{\"event\":\"subagent_start\",\"agent_name\":{f}}}", .{std.json.fmt(e.agent_name, .{})}) catch return,
...
pub const OtelObserver = struct {
    ...
    fn otelRecordEvent(ptr: *anyopaque, event: *const ObserverEvent) void {
        ...
        switch (event.*) {
            ...
            .subagent_start => |e| {
                self.addSpan("subagent.start", now, now, &.{
                    .{ .key = "agent_name", .value = e.agent_name },
                });
            },
            .cron_job_start => |e| {
                self.startCurrentTrace(now);
